...
const RenderCell = ({ data }) => {
  ...
  const holidayFound = day
    ? holidays.find((iter) => cellDate === iter.date)
    : undefined;
  ...
  const currentDay =
    formatDate(state.currentDate) === cellDate
      ? { backgroundColor: "rgba(228, 107, 103, 0.4)" }
      : {};
  ...
  const cells = (header, days) => {
    ...
    return (
      <div style={styles.flexRow}>
        <RenderCell
          data={{ ...ym, day: days[0], header: header ? "sunday" : "" }}
        />
        <RenderCell
          data={{ ...ym, day: days[1], header: header ? "monday" : "" }}
        />
        <RenderCell
          data={{ ...ym, day: days[2], header: header ? "tuesday" : "" }}
        />
        <RenderCell
          data={{ ...ym, day: days[3], header: header ? "wednesday" : "" }}
        />
        <RenderCell
          data={{ ...ym, day: days[4], header: header ? "thursday" : "" }}
        />
        <RenderCell
          data={{ ...ym, day: days[5], header: header ? "friday" : "" }}
        />
        <RenderCell
          data={{ ...ym, day: days[6], header: header ? "saturday" : "" }}
        />
      </div>
    );
  };
...
0's for cells that aren't being used by the current month/year being displayed */
  const rows = genDaysArr(
    monthObj[1].days,
    firstDay,
    leapYear(state.selectedDate.getFullYear())
  );
  ...
  const sorted = Object.entries(state.provinces).sort((textA, textB) =>
    ...
    <div style={styles.flexC}>
      {
        <div style={styles.flexRow}>
          ...
          <h2
            ...
            style={styles.cursorLeft}
          >
            {"<"}
          </h2>
          <h2 style={styles.currentMonth}>{`${capitalize(
            monthObj[0]
          )} ${state.selectedDate.getFullYear()}`}</h2>
          <h2
            onClick={() => {
              onArrowClick("right");
            }}
            style={styles.cursorRight}
          >
            {">"}
          </h2>
